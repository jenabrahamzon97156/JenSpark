...
import { searchFoods, OffSearchResult } from "@/lib/openFoodFacts";
import { sumFoods } from "@/lib/foodStore";
...
function QuantityPicker({
  ...
  return (
    <div className="rounded-lg border border-[#4C6EF5] bg-[#4C6EF5]/5 p-3 mt-2">
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 bg-white border border-[#E5E7EB] rounded-md px-3 py-2 text-sm font-mono text-[#1D2027] focus:outline-none focus:border-[#4C6EF5]"
        />
        <button
          onClick={() => onConfirm(Number(qty) || 1)}
          className="px-4 rounded-md bg-[#4C6EF5] text-white text-sm font-medium"
        >
          Add
        </button>
...
export default function AddFoodPanel({
  ...
  onLogRecipe: (recipe: RecipeWithIngredients, servingsEaten: number) => void;
  onSaveSearchResult: (result: OffSearchResult) => Promise<FoodItem | null>;
  onCreateManualFood: (food: Omit<FoodItem, "id">) => void;
  onClose: () => void;
}) {
  ...
  const [pickingResult, setPickingResult] = useState<OffSearchResult | null>(null);
        ...
        <div>
          <div className="flex gap-2 mb-3">
            <input
              ...
              className="flex-1 bg-[#F7F8FA] border border-[#E5E7EB] rounded-md px-3 py-2 text-sm text-[#1D2027] focus:outline-none focus:border-[#4C6EF5]"
            />
            <button onClick={runSearch} className="px-4 rounded-md bg-[#4C6EF5] text-white text-sm font-medium">
              Go
            </button>
          </div>
              ...
              <div key={i} className="rounded-lg border border-[#E5E7EB] p-2.5">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="shrink-0 text-xs px-3 py-1 rounded-full bg-[#4C6EF5] text-white"
                  >
                    Add
                  </button>
                </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <button
            onClick={() => {
              ...
              setManual({ name: "", servingQty: "1", servingUnit: "serving", calories: "", proteinG: "", fiberG: "", sugarG: "" });
            }}
            className="rounded-md bg-[#4C6EF5] text-white text-sm font-medium py-2 mt-1"
          >
            Save &amp; add to today
          </button>
        </div>
